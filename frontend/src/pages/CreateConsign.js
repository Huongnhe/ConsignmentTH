...
const CreateConsign = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Gọi API để tạo đơn ký gửi
            const response = await createConsign(token, consignmentData);

            // Nếu thành công, thông báo và chuyển hướng về trang danh sách ký gửi
    ...
    return (
        <div>
            <NavbarUser /> {/* Hiển thị navbar người dùng */}
            <div className="container mt-5">
                <h2 className="text-center text-primary">Tạo Đơn Ký Gửi</h2>
                {error && <p className="text-danger text-center">{error}</p>} {/* Hiển thị lỗi nếu có */}
